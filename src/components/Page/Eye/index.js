// https://cssgradient.io/
import React, { PureComponent } from 'react';
import { compose } from 'redux';
import { sample } from 'lodash-es';

import Layer from '../../Layer';
import Graphics from '../../Graphics';
import Main from '../../Main';
import Header from '../../Header';
import Footer from '../../Footer';
import withTitle from '../../../hocs/withTitle';
import withVariables from '../../../hocs/withVariables';

import './index.css';

const TYPES = {
  HUMAN: 'HUMAN',
};
const BACKGROUND_COLOR = '#393232';
const NOISE = 'data:image/png;base64,';
const EYES = [
  {
    id: 1,
    type: TYPES.HUMAN,
    COLOR_1:'rgba(255,255,255,.5)',
    COLOR_2: `url(${NOISE}), radial-gradient(circle, rgba(161,111,74,1) 30%, rgba(0,0,0,1) 100%)`,
    COLOR_3: 'rgba(0, 0, 0, .75)',
    COLOR_4: 'rgba(255, 255, 255, .55)',
  },
  {
    id: 2,
    type: TYPES.HUMAN,
    COLOR_1:'rgba(255,255,255,.5)',
    COLOR_2: `url(${NOISE}), radial-gradient(circle, rgba(63,94,251,1) 0%, rgba(98,60,14,1) 100%)`,
    COLOR_3: 'rgba(0, 0, 0, .75)',
    COLOR_4: 'rgba(255, 255, 255, .55)',
  },
  {
    id: 3,
    type: TYPES.HUMAN,
    COLOR_1:'rgba(255,255,255,.75)',
    COLOR_2: `url(${NOISE}), radial-gradient(circle, rgba(255,255,255,1) 21%, rgba(232,104,104,1) 78%, rgba(66,44,44,1) 88%)`,
    COLOR_3: '#db5e5e',
    COLOR_4: 'rgba(255, 255, 255, .55)',
  },
];

class PageMagic8Ball extends PureComponent {
  constructor(props) {
    super(props);
    this.state = { mouse: false, eye: sample(EYES) };
    this.timeout = null;
  }

  componentDidMount() {
    document.addEventListener('mousemove', this.onMouseMove);
  }

  componentWillUnmount() {
    document.removeEventListener('mousemove', this.onMouseMove);
  }

  onMouseMove = () => {
    if (this.timeout) clearTimeout(this.timeout);
    this.setState({ mouse: true });

    this.timeout = setTimeout(() => {
      this.setState({ mouse: false });
    }, 1000);
  }

  render() {
    const { eye } = this.state;

    return (
      <>
        <Header />

        <Main>
          <Graphics type="perspective" className={this.state.mouse ? '' : 'Eye' } transition>
            <Layer l="1.5">
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%', height: '100%' }}>
                <div
                  style={{
                    width: '35vmin',
                    height: '35vmin',
                    borderRadius: '50%',
                    backgroundImage: `url(${NOISE})`,
                    backgroundColor: eye.COLOR_1,
                    boxShadow: 'inset 0 0 5px 10px rgba(255,255,255,.25)',
                  }}
                />
              </div>
            </Layer>
            <Layer l="3">
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%', height: '100%' }}>
                <div
                  style={{
                    width: '17.5vmin',
                    height: '17.5vmin',
                    borderRadius: '50%',
                    background: eye.COLOR_2,
                    display: 'flex',
                  }}
                />
              </div>
            </Layer>
            <Layer l="4.5">
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%', height: '100%' }}>
                <div
                  style={{
                    width: '8vmin',
                    height: '8vmin',
                    borderRadius: '50%',
                    background: eye.COLOR_3,
                  }}
                >
                  <div style={{
                    background: eye.COLOR_4,
                    width: 40,
                    height: 30,
                    borderRadius: '50%',
                  }} />
                </div>
              </div>
            </Layer>
          </Graphics>
        </Main>

        <Footer strings={this._strings} />
      </>
    );
  }
}

export default compose(
  withVariables({ '--background-color': BACKGROUND_COLOR }),
  withTitle('The Eye'),
)(PageMagic8Ball);
